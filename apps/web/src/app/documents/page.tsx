...
export default function DocumentsPage() {
  ...
  async function saveStyle() {
    if (!selected) return;
    setBusy("save");
    const style = { ...selected, name: saveName.trim() || selected.name };
      ...
      <div className="max-w-5xl mx-auto p-6 flex flex-col gap-6">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: "var(--text-3)" }}>
            Elige o extrae un estilo, ajústalo, guárdalo, y genera ofertas con esa apariencia.
          </p>
        </div>
        ...
        <section className="card">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
            {entries.map((e) => (
              <button key={e.name + (e.builtin ? "-b" : "")} onClick={() => setSelected(e.style)}
                ...
                <div className="flex items-center justify-between mt-2">
                  ...
                  {!e.builtin && (
                    <span role="button" onClick={(ev) => { ev.stopPropagation(); removeStyle(e.name); }}
                      className="text-xs opacity-50 hover:opacity-100" style={{ color: "var(--danger)" }}>✕</span>
                  )}
                </div>
